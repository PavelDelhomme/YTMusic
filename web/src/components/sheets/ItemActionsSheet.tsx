...
import { applySleepPick, SLEEP_TIMER_OPTIONS } from '../../lib/player/sleepTimer';
...
import { ArtistLinks } from '../media/ArtistLinks';
...
export function ItemActionsSheet({ onOpenEqualizer }: { onOpenEqualizer?: () => void }) {
  ...
  const [containedPlaylistIds, setContainedPlaylistIds] = useState<Set<string>>(new Set());
  ...
  const addTrackToPlaylist = async (playlistId: string) => {
    ...
    try {
      ...
      setContainedPlaylistIds((prev) => new Set([...prev, playlistId]));
      ...
      <div className="relative z-10 max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-t-2xl border border-yt-border bg-yt-elevated shadow-2xl animate-fade-up sm:rounded-2xl">
        ...
        {showPlaylists && (
          <div className="border-b border-white/10 px-2 py-2">
            ...
            {playlists.map((p) => {
              const already =
                containedPlaylistIds.has(p.id) ||
                (p.tracks || []).some((t) => t.id === item.id);
              return (
                <Row
                  key={p.id}
                  icon={
                  ...
                  disabled={busy || already || !membershipReady}
                  onClick={() => {
                    if (already || busy || !membershipReady) return;

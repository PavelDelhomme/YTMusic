...
deploy-hint: ## Guide déploiement Portainer / NPM / mobile
	...
	@echo "  --------------------------------------"
	@echo "  DNS  ytmusic.delhomme.ovh → IP VPS (sinon NXDOMAIN)"
	@echo ""
	@echo "  WEB  1) git push origin prod  → image GHCR :latest"
	@echo "       2) Portainer : coller deploy/portainer-template.yml"
	@echo "          ou Git URL=https://github.com/PavelDelhomme/YTMusic"
	@echo "             Ref=refs/heads/prod  Compose=docker-compose.yml"
	@echo "       3) Env : JWT_SECRET SMTP_PASS ADMIN_EMAILS AUTH_ALLOWED_EMAILS"
	@echo "                AUTH_ALLOW_REGISTER=0 AUTH_ALLOW_GUEST=0"
	@echo "       4) NPM : ytmusic.delhomme.ovh → ytmusic:8787 + Websockets ON"
	@echo ""

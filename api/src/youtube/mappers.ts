...
/** Titre « musique » : enlève Official Video / Officiel / Lyrics / 4K… (clips YT). */
export function cleanMusicTitle(raw: string): string {
  ...
  t = t
    ...
    .replace(
      /\s*[\[(【]\s*(clip\s*)?officiel(?:le)?(\s*(?:vid[eé]o|audio|lyrics?))?\s*[\]】)]/gi,
      '',
    )
    .replace(
    ...
    // Suffixes hors crochets : « Titre - Official Video », « Titre | Clip Officiel », « Titre Officiel »
    .replace(
      /\s*[-–—|/]\s*(official(\s*(music\s*)?(video|audio|lyric\s*video))?|clip\s*officiel(?:le)?|officiel(?:le)?|official\s*video)\s*$/gi,
      '',
    )
    .replace(/\s+(official(\s*video)?|clip\s*officiel(?:le)?|officiel(?:le)?)\s*$/gi, '')
    .replace(/\s{2,}/g, ' ')
    .replace(/\s*[-–—]\s*$/g, '')
    .trim();
...
/** Clé de dédoublonnage artiste+titre (pour préférer audio vs clip). */
export function trackCatalogKey(t: Track): string {
  const title = cleanMusicTitle(t.title || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/\p{M}/gu, '')
    .replace(/[^a-z0-9]+/g, ' ')
...
/**
 * Score « catalogue audio » : ATV / song > OMV / clip.
 * Plus haut = mieux pour « À suivre » / related (style YTM audio-first).
 */
export function audioCatalogScore(t: Track): number {
  let s = 0;
  const typ = String(t.type || '');
  if (typ === 'song') s += 80;
  else if (typ === 'video') s -= 120;
  const rawTitle = String(t.title || '');
  // Titre déjà nettoyé : on regarde aussi si le nettoyage a « perdu » des indices via durée typique clip
  if (/\b(official|officiel|clip)\b/i.test(rawTitle)) s -= 60;
  const dur = Number(t.durationSeconds) || 0;
  // Clips souvent plus longs (intro/outro) ; léger malus si > 7 min sans type song clair
  if (dur > 420 && typ !== 'song') s -= 20;
  if (dur > 0 && dur < 90 && typ === 'video') s -= 30; // short promo
  return s;
}

/**
 * Préfère les pistes audio (ATV) aux clips officiels pour un même titre+artiste.
 * Conserve l’ordre relatif sinon.
 */
export function preferCatalogAudio(tracks: Track[]): Track[] {
  const best = new Map<string, { track: Track; score: number; idx: number }>();
  tracks.forEach((t, idx) => {
    if (!t?.id) return;
    const key = trackCatalogKey(t);
    // Pas de titre exploitable → clé unique par id
    const k = key.endsWith('|') || key.startsWith('|') || key === '|' ? `id:${t.id}` : key;
    const score = audioCatalogScore(t);
    const prev = best.get(k);
    if (!prev || score > prev.score || (score === prev.score && idx < prev.idx)) {
      best.set(k, { track: t, score, idx });
    }
  });
  // Reconstruire : d’abord les gagnants dédupliqués, triés par score puis ordre d’apparition
  const winners = [...best.values()].sort((a, b) => b.score - a.score || a.idx - b.idx);
  // Aussi remonter les song/ATV en tête parmi le reste (sans casser la diversité trop fort)
  const songFirst = winners.filter((w) => w.score >= 40);
  const rest = winners.filter((w) => w.score < 40);
  return [...songFirst, ...rest].map((w) => w.track);
}

...
# Usage: DEVICE=R5CT7263YJL bash scripts/mobile-full-smoke.sh
set -euo pipefail
...
inject_session() {
  local api="$1"
  bash "$ROOT/scripts/adb-login.sh" >/dev/null
...
run_env() {
  ...
  DEVICE="$DEVICE" API_BASE_URL="$api" bash "$ROOT/scripts/adb-login.sh" | tee -a "$REPORT"
  sleep 3
...
if ! curl -sf http://127.0.0.1:8787/api/health >/dev/null; then
  ...
  bash "$ROOT/scripts/ensure-api.sh" >/dev/null || true
fi
...
set_api_override "http://127.0.0.1:8787"
$ADB -s "$DEVICE" reverse tcp:8787 tcp:8787 || true
DEVICE="$DEVICE" API_BASE_URL="http://127.0.0.1:8787" bash "$ROOT/scripts/adb-login.sh" >/dev/null || true

...
warn_cookies() {
  ...
  echo "   Les streams marchent souvent sans. Stabiliser / VPS : bash scripts/deploy/push-youtube-cookies.sh local"
  ...
  if [[ "$try_auto" == "1" && "${AUTO_YT_COOKIES:-0}" == "1" ]]; then
    ...
    if timeout 45 bash "$ROOT/scripts/deploy/push-youtube-cookies.sh" local >>"$LOG" 2>&1; then

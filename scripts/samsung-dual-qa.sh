...
#
# Usage:
#   DEVICE=R5CT7263YJL bash scripts/samsung-dual-qa.sh
#   ONLY=dev|prod bash scripts/samsung-dual-qa.sh
set -euo pipefail
...
mobile_flow() {
  ...
  sleep 1
  DEVICE="$DEVICE" API_BASE_URL="$api" PKG="$pkg" bash "$ROOT/scripts/adb-login.sh" | tee -a "$REPORT"
  sleep 3
  dump_ui "$tag-home"
  local texts

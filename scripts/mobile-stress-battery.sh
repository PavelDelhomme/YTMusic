...
# Usage:
#   DEVICE=R5CT7263YJL bash scripts/mobile-stress-battery.sh
#   DEVICE=00145153K001434 API_BASE_URL=https://ytmusic.delhomme.ovh bash scripts/mobile-stress-battery.sh
set -euo pipefail
...
if [[ -f "$ROOT/scripts/adb-login.sh" ]]; then
  DEVICE="$DEVICE" API_BASE_URL="${API_BASE_URL:-}" bash "$ROOT/scripts/adb-login.sh" >>"$REPORT" 2>&1 || true
  sleep 2
fi
